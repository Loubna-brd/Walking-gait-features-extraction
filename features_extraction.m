function [table_features,C_accel] = features_extraction(heartrate,stepping_period_accel_filt,loc_peak,time_peak,section_beg,section_end,a_model,b_model,l0)

% Extraction of the features from n strides in each walking period
% Input:
%   - heartrate: heartrate file to extract heartrate as a feature
...
% sanity check to visualize peak detection:
% if length(loc_peak)>=10
%     figure()
%     plot(loc_peak{10},peak_value{10},'ko','MarkerFaceColor','r');
%     hold on
%     plot(smooth(stepping_period_accel_filt{10}(:,3),'lowess'))
%     hold on
%     vline(section_beg{10},'r')
%     hold on
%     vline(section_end{10},'r:')
%     xlabel('time index')
%     ylabel('acceleration [m/s2]')
% end


M_features = zeros(43,20000);
...
for k1 = 1:length(stepping_period_accel_filt)
    ...
    for i = 1:size(M_loc_n,1)
        ...
        M_features(:,i1) = [i1; k1; M_t_pks_n(i,1); freq_norm;stride_time_norm;speed_norm;hr_average;features_raw_acc_array];
...
colNames = {'index','bout_number','time_first_stride',...
    ...
    'normalized_stride_speed','average_hr',...
